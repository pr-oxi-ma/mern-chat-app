...
// ------------------------- CREATE CHAT -------------------------
const createChat = asyncErrorHandler(
  async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
    ...
    if (isGroupChat === "true") {
      if (members.length < 2) {
        return next(
          new CustomError(
            "At least 2 members are required to create a group chat",
            400
          )
        );
      } else if (!name) {
        return next(
          new CustomError("Name is required for creating group chat", 400)
        );
      }
      ...
      const avatar =
        hasAvatar && uploadResults && uploadResults[0]
          ? uploadResults[0].secure_url
          : DEFAULT_AVATAR;
      const avatarCloudinaryPublicId =
        hasAvatar && uploadResults && uploadResults[0]
...
// ------------------------- GET USER CHATS -------------------------
const getUserChats = asyncErrorHandler(
...
// ------------------------- ADD MEMBER TO CHAT -------------------------
const addMemberToChat = asyncErrorHandler(
  async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
    const { id } = req.params;
    ...
    if (!chat) return next(new CustomError("Chat does not exist", 404));
    if (!chat.isGroupChat)
      return next(
        new CustomError("This is not a group chat, you cannot add members", 400)
      );

    if (chat.adminId !== req.user.id)
      return next(
        new CustomError("You are not allowed to add members", 400)
      );

    const areMembersToBeAddedAlreadyExists = await prisma.chatMembers.findMany({
      where: { chatId: id, userId: { in: members } },
    ...
    if (areMembersToBeAddedAlreadyExists.length) {
      return next(
        new CustomError(
          `${areMembersToBeAddedAlreadyExists
            .map(({ user: { username } }) => `${username}`)
            .join(", ")} already exists in chat`,
          400
        )
      );
    }
    ...
    const oldExistingMembersIds = oldExistingMembers.map(
      ({ user: { id } }) => id
    );

    await prisma.chatMembers.createMany({
      data: members.map((memberid) => ({ chatId: id, userId: memberid })),
    });
...
const removeMemberFromChat = asyncErrorHandler(
  async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
    const { id } = req.params;
    ...
    if (!chat.isGroupChat)
      return next(
        new CustomError("This is not a group chat, you cannot remove members", 400)
      );

    if (req.user.id !== chat.adminId)
      return next(
        new CustomError("You are not allowed to remove members", 400)
      );
    ...
    if (existingMembers.length === 3)
      return next(
        new CustomError("Minimum 3 members are required in a group chat", 400)
      );
    ...
    if (invalidMembers.length)
      return next(
        new CustomError("Provided members do not exist in chat", 404)
      );
    ...
    const deletedChatPayload: DeleteChatEventSendPayload = { chatId: id };
    emitEvent({
      io,
    ...
    const remainingMembers = existingMemberIds.filter(
...
// ------------------------- UPDATE CHAT -------------------------
const updateChat = asyncErrorHandler(
  async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
    const { id } = req.params;
    ...
    if (!name && !avatar)
      return next(
        new CustomError(
          "Either avatar or name is required for updating a chat"
        )
      );
    ...
    if (!chat.isGroupChat)
      return next(new CustomError("Cannot update a private chat", 400));

    if (avatar) {
      if (chat.avatarCloudinaryPublicId) {
        await deleteFilesFromCloudinary({
          publicIds: [chat.avatarCloudinaryPublicId],
        });
      }
    ...
    if (name) {
      await prisma.chat.update({ where: { id }, data: { name } });
    }
