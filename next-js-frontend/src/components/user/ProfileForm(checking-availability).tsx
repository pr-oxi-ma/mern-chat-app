...
/**
 * ProfileForm
 * - Now includes Real-time Username Availability Check
 */
...
export const ProfileForm: React.FC = () => {
  ...
  useEffect(() => {
    // 1. Don't check if empty or too short
    if (!username || username.length < 3) {
      setIsCheckingUsername(false);
      return;
    }

    // 2. Don't check if it's the user's current username
    if (username === loggedInUser?.username) {
      setIsCheckingUsername(false);
      // Clear error if they typed it back to their original name
    ...
    const delayDebounceFn = setTimeout(async () => {
      ...
      try {
        ...
        if (!data.available) {
          ...
          // Only clear error if it was a "taken" error. 
          // If there's a different error (like "invalid chars"), we might want to keep logic separate,
          // but here we assume the API covers validity too.
          setUsernameError(null);
        }
      } catch (error) {
        console.error("Error checking username availability", error);
        // Optional: setUsernameError("Could not verify username");
      } finally {
        setIsCheckingUsername(false);
      }
    }, 500); // 500ms delay
  ...
  const handleUpdateProfile = async () => {
    // If we are still checking username, do not submit
    ...
    // Final safeguard before submit in case the async check failed earlier
    ...
    <div className="p-4 max-w-2xl mx-auto text-gray-900 dark:text-gray-100 relative">
      ...
      <div className="flex flex-col items-center gap-y-4">
        ...
        <div className="w-full max-w-md relative">
          ...
          <input
            ...
            onChange={(e) => {
              const cleaned = e.target.value
                ...
                .replace(/[^a-z0-9_]/g, "");
              setUsername(cleaned);
              setUsernameError(null); // Clear error immediately on type, wait for debounce to re-check
            }}
            onKeyDown={(e) => {
          ...
          <div className="absolute right-2 top-1/2 transform -translate-y-1/2 mt-4">
            <PencilIcon
        ...
        {hasUnsavedChanges && (
          <div className="flex flex-col items-center gap-2">
            ...
            <button
              ...
              // Disable if loading (submitting) OR if checking username availability OR if there is a known username error
